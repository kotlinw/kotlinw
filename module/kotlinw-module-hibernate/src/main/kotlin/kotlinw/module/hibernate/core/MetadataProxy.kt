...
internal class MetadataProxy: Metadata {
    ...
    override fun getReferencedPropertyType(className: String?, propertyName: String?): Type = delegate.getReferencedPropertyType(className, propertyName)
    ...
    override fun getNamedHqlQueryMapping(name: String?): NamedHqlQueryDefinition  = delegate.getNamedHqlQueryMapping(name)

    override fun visitNamedHqlQueryDefinitions(definitionConsumer: Consumer<NamedHqlQueryDefinition>?)  = delegate.visitNamedHqlQueryDefinitions(definitionConsumer)

    override fun getNamedNativeQueryMapping(name: String?): NamedNativeQueryDefinition  = delegate.getNamedNativeQueryMapping(name)

    override fun visitNamedNativeQueryDefinitions(definitionConsumer: Consumer<NamedNativeQueryDefinition>?)  = delegate.visitNamedNativeQueryDefinitions(definitionConsumer)

    override fun getNamedProcedureCallMapping(name: String?): NamedProcedureCallDefinition  = delegate.getNamedProcedureCallMapping(name)

    override fun visitNamedProcedureCallDefinition(definitionConsumer: Consumer<NamedProcedureCallDefinition>?)  = delegate.visitNamedProcedureCallDefinition(definitionConsumer)

    override fun getResultSetMapping(name: String?): NamedResultSetMappingDescriptor  = delegate.getResultSetMapping(name)

    override fun visitNamedResultSetMappingDefinition(definitionConsumer: Consumer<NamedResultSetMappingDescriptor>?)  = delegate.visitNamedResultSetMappingDefinition(definitionConsumer)
    ...
    override fun getIdentifierGenerator(name: String?): IdentifierGeneratorDefinition  = delegate.getIdentifierGenerator(name)
